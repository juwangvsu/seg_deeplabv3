...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if ddp:
            # Sum confusion matrices across ranks
            hist_t = torch.tensor(hist, device=device, dtype=torch.float64)
            dist.all_reduce(hist_t, op=dist.ReduceOp.SUM)
            hist = hist_t.cpu().numpy()

        # Average val_loss across ranks (sum loss, sum batches)
        loss_and_batches = torch.tensor([val_loss, len(val_loader)], device=device, dtype=torch.float64)
        dist.all_reduce(loss_and_batches, op=dist.ReduceOp.SUM)
        val_loss = (loss_and_batches[0] / torch.clamp(loss_and_batches[1], min=1)).item()
        ...
        '''
        # Save per-epoch
        if ((epoch + 1) % max(1, args.save_every) == 0) and is_main_process():
        ...
        # Save best (by mIoU)
        if miou > best_miou and is_main_process():
            best_miou = miou
