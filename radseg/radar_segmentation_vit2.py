...
def evaluate(model, loader, device, num_classes, ignore_index, save_output: bool = False, out_dir: str | None = None):
    ...
    for batch in loader:
        ...
        if save_output and out_dir is not None:
            # Save each prediction as 32-bit PNG of class indices
            for i in range(pred.size(0)):
                p = pred[i].detach().cpu().to(torch.int32).numpy()
                im = Image.fromarray(p, mode="I")
                base = Path(batch["path"][i]).stem if isinstance(batch["path"], list) or isinstance(batch["path"], tuple) else Path(batch["path"]).stem
                im.save(os.path.join(out_dir, f"{base}.png"))
                counter += 1
...
def parse_args():
    ...
    p.add_argument("--ckpt", type=str, default="ckpt.pt", help="Checkpoint filename (within ckpt-dir)")
    p.add_argument("--save-every", type=int, default=5, help="Save checkpoint every N epochs (0 disables)")
...
def _infer_on_file(model, device, npy_path: str, img_size: Tuple[int, int], normalize: str, out_png: str | None = None):
    ...
    x_t = torch.from_numpy(arr).unsqueeze(0).unsqueeze(0)  # [1,1,H,W]
...
def main():
    ...
    if args.mode == "train":
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if args.save_every and (epoch % args.save_every == 0):
                ...
                print(f"Saved checkpoint (periodic) to {ckpt_path}")

            # best save
            if miou > best_miou:
                best_miou = miou
                torch.save(model.state_dict(), ckpt_path)
                print(f"Saved checkpoint (best mIoU {best_miou:.4f}) to {ckpt_path}")
        return
    ...
    if args.mode == "infer":
        ...
        if len(npy_files) == 0:
            ...
            return
        out_dir = os.path.join(args.ckpt_dir, "preds_infer")
        os.makedirs(out_dir, exist_ok=True)
        for f in npy_files:
            base = Path(f).stem
            out_png = os.path.join(out_dir, f"{base}.png")
            _ = _infer_on_file(model, device, f, tuple(args.img_size), normalize="per_image_zscore", out_png=out_png)
